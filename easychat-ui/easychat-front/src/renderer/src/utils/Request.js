...
instance.interceptors.response.use(
    (response) => {
        ...
        if (responData.code == 200) {
            ...
            //其他错误
            if (errorCallback) {
                errorCallback(responData)
            }
            return Promise.reject({ showError, msg: responData.info })
        }
    },
    (error) => {
...
const request = (config) => {
    const { url, params, dataType, showLoading = true, responseType = responseTypeJson, showError = true } =
        config
